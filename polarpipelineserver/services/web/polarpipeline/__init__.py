...
FIGURE_PRESETS_CONFIG = './resources/presets.ini'

base_path = '/usr/src/app/input'

@app.route('/browse/<path:path>')
@app.route('/')
def browse(path=None):

    print(os.listdir('/usr/src/app/polarpipeline'))
    if path is None or 'input' not in path:
        path = base_path

    full_path = os.path.join('/usr/src/app/', path)
    directory_listing = {}
    ...
    bed_files = os.listdir('./resources/bed_files')
    gene_sources = os.listdir('./resources/gene_source')
    clair_models = os.listdir('./resources/clair_models')
    reference_files = os.listdir('./resources/reference_files')
    for item in reference_files:
        if item.endswith('.fai') or item.endswith('.index'):
            reference_files.remove(item)
    
    full_path = full_path.replace('/usr/src/app', '')
...
def trigger_processing():
    ...
    if grch_reference != 'none':
        ...
        try:
            ...
            conn.rollback()
        cursor.close()
        print(f'process.delay({path}, {clair_model}, {grch_gene}, {grch_bed}, {grch_reference}, {id})')
...
def upload(filetype):
    ...
    save_directory = os.path.join(f"./resources/", filetype)
...
def generatefigure():
    ...
    with open('./polarpipeline/static/variantFig.svg', 'w') as opened:
