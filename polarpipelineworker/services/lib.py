...
#   returns: new filepath of the bam, or False if it fails.
    try:
        samtools_folder = '/usr/src/app/tools/samtools'
        samtools_path = ''
        for root,dirs,files in os.walk(samtools_folder):
            for file in files:
                if file == 'minimap2':
                    samtools_path = os.path.join(root, file)
                    break
        ...
        if input_file.endswith('.fastq.gz'):
            ...
            stdout, stderr = process.communicate()
        process = subprocess.Popen([samtools_path, "import", "-@", "30" "-0", run_name+".fastq", "-o", run_name+".bam"], cwd=working_path)
        stdout, stderr = process.communicate()
...
def minimap2(input_path, reference_path, threads='30'):
    minimap_folder = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'tools/minimap')
    minimap_path = ''
    for root,dirs,files in os.walk(minimap_folder):
        for file in files:
            if file == 'minimap2':
                minimap_path = os.path.join(root, file)
                break
    root = input_path.split('.fastq')[0]
    minimap_command = f'{minimap_path} -y -t {threads} -ax map-ont {reference_path} {root}.fastq > {root}.sam'
...
def viewSortIndex(input_path, threads='30'):
    samtools_folder = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'tools/samtools')
    samtools_path = ''
    for root,dirs,files in os.walk(samtools_folder):
        for file in files:
            if file == 'samtools' and 'bin' in root:
                samtools_path = os.path.join(root, file)
                break

    root = input_path.split('.sam')[0]

    view_command = f'{samtools_path} view -@ {threads} -bo {root}.bam {root}.sam'
    os.system(view_command)
    # print(view_command)

    sort_command = f'{samtools_path} sort -m 2G -o {root}_sorted.bam -@ {threads} {root}.bam'
    os.system(sort_command)
    # print(sort_command)

    index_command = f'{samtools_path} index -b -@ {threads} -o {root}_sorted.bam.bai {root}_sorted.bam'
...
#   clair3_model_path: full path to the clair3 model folder
    ...
    nextflow_location = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'tools/nextflow')
    command = f"{nextflow_location}/nextflow run epi2me-labs/wf-human-variation \
...
#   return: none. does more damage the more the user likes you.
    ...
    pc_name = whoami()
    run_name = os.path.basename(input_snv).split('.sepAlt.')[0]
    ...
    start = f'{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/ensembl-vep/vep --offline --cache --tab --everything --assembly GRCh38 --fasta {reference_path} --fork {threads} --buffer_size 20000'
    params = [
        ' --sift b',
        ' --polyphen b',
        ' --ccds',
    ...
    plugins = [
        f' --plugin LoFtool,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/LoFtool_scores.txt',
        f' --plugin Mastermind,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/mastermind_cited_variants_reference-2023.04.02-grch38.vcf.gz',
        f' --plugin CADD,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/whole_genome_SNVs.tsv.gz',
        f' --plugin Carol',
        f' --plugin Condel,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/.vep/Plugins/config/Condel/config',
        f' --plugin pLI,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/pLI_values.txt',
        f' --plugin PrimateAI,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/PrimateAI_scores_v0.2_GRCh38_sorted.tsv.bgz',
        f' --plugin dbNSFP,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/dbNSFP4.4a_grch38.gz,ALL',
        f' --plugin REVEL,{os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/new_tabbed_revel_grch38.tsv.gz',
        f' --plugin AlphaMissense,file={os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/AlphaMissense_hg38.tsv.gz',
        f' --plugin EVE,file={os.path.split(os.path.abspath(os.path.curdir))[0]}/tools/vep/vep-resources/eve_merged.vcf.gz'
    ]
    
    input_dir = os.path.split(input_snv)[0]
