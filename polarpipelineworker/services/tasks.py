...
app = Celery('tasks', broker=f'pyamqp://guest:guest@{os.environ.get("SERVER_IP")}:5672/')

@app.task(name='polarpipeline.tasks.process')
def process(input_file_path, clair_model_name, gene_source_name, bed_file_name, reference_file_name, id):

    og_input = input_file_path
    input_file_path = os.path.split(os.path.abspath(os.path.curdir))[0] + og_input
    print(input_file_path)

    clair_model_path = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'resources/clair_models', clair_model_name)
    gene_source_path = []
    for name in gene_source_name:
        gene_source_path.append(os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'resources/gene_source',name))
    reference_path = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'resources/reference_files', reference_file_name)
    bed_file_path = []
    for name in bed_file_name:
        bed_file_path.append(os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'resources/bed_files',name))

    pc_name = whoami()

    CONFIG_FILE_PATH = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'resources/config.ini')

    config = configparser.ConfigParser()

    config.read(CONFIG_FILE_PATH)

    if not os.path.isdir(os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0],'output', config['General']['output_directory'])):
        update_db(id, 'status', 'output path not found')
        quit()
    else: print(f'config defined output path: {config["General"]["output_directory"]}', end='\n')
    ...
    run_name = os.path.basename(input_file_path).split('.bam')[0].split('.fastq')[0]
    ...
    working_path = f'{os.path.split(os.path.abspath(os.path.curdir))[0]}/workspace/{id}'
    ...
    output_dir = os.path.join(os.path.split(os.path.abspath(os.path.curdir))[0], 'output' + config['General']['output_directory'],f'{formattedTime}_{run_name}')
...
@app.task(name='polarpipeline.tasks.processT2T')
